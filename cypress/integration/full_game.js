...
describe('Play through age 1', () => {
	it('Open and closes modal, adds civ card', () => {
		// Correct disabled count
		cy.get('[data-blocked]').should('have.length', 14);
		...
		cy.get('.player.me .cards .card-sm').should('have.length', 1);
		cy.get('[data-id="altar"]').should('exist');
	...
	it('Counts cards correctly so far', () => {
		...
		cy.get('.player.me .score').should('have.text', '6 5');
	...
	it('War progress reduces opponent coins', () => {
		...
		cy.get('.penalty.top-2.got').should('exist');
	});
...
describe('Play through age 2', () => {
	...
	it('Buy matching science icon earns a token', () => {
		...
		cy.get('.modal [data-id="bellafide"]').click();
		cy.get('.player.me .token[data-id="bellafide"]').should('exist');
		cy.get('aside.tokens .token[data-id="bellafide"][data-taken]').should('exist');

		cy.get('[data-index="9"]').click();
		cy.contains('Get for Free').click();

		cy.get('.modal [data-id="hopper"]').click();
		cy.get('.player.you .token[data-id="hopper"]').should('exist');
		cy.get('aside.tokens .token[data-id="hopper"][data-taken]').should('exist');
	});
	...
	it('Economy token works', () => {
		cy.get('[data-index="7"]').click();
		cy.get('.player.me .score').should('have.text', '13 3');
		cy.contains('Buy for 2').click();
		cy.get('.player.me .score').should('have.text', '13 5');
	});
	...
	it('Should not hit negatives when war nukes coins', () => {
		...
		cy.get('.penalty.top-5.got').should('exist');
	});
...
describe('Play through age 3', () => {
	...
	it('Instant Eco card gains ingoo/VP (Wonders)', () => {
		...
		cy.get('[data-index="25"]').click();
		cy.get('.player.me .score').should('have.text', '18 4');
		cy.contains('Get for Free').click();
		cy.get('.player.me .score').should('have.text', '21 8');
	});
	...
	it('Wonder provides resources', () => {
		...
		cy.contains('Buy for 3').click();
		cy.get('.player.me .score').should('have.text', '24 9');
	...
	it('Destroy Coin ability works', () => {
		...
		cy.get('.player.me .score').should('have.text', '34 7');
	});
